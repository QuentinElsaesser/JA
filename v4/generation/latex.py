...
class Latex:
    def __init__(self, nb_metrics, spe, path_f="../results/", nvalue="PRC", fixed_prc="", infograph="", readname="", distrib=""):
        """
        ...
        self.nvalue = nvalue
        self.fixed_prc = ""
        if nvalue == "VRN" or nvalue == "SPR" or nvalue == "SDR":
    ...
    def texte(self, prc, fixed_prc, first):
        if self.nvalue == "PRC" or self.nvalue == "PRP":
            return f"{prc}\%"
        elif self.nvalue == "VRN":
            return f"variance +/- {prc}"
        else:
            if self.nvalue == "FCT":
                txt = "fct"
            elif self.nvalue == "SRC" or self.nvalue == "SPR" or self.nvalue == "SDR":
                txt = "src"
            elif self.nvalue == "OBJ":
                txt = "obj"
    ...
    def start_file(self):
        ...
        self.lxt += "\\newcommand{\\distrib}{" + self.distrib + "}\n"
        self.lxt += "\\newcommand{\\graph}[2]{$G_{#1}^{#2}$}\n"
        fprc = ""
        if self.fixed_prc != "":
            fprc = f"_{self.fixed_prc}"
        self.lxt += "\\newcommand{\\typegeneration}[0]{" + self.nvalue + fprc+ "}\n"
