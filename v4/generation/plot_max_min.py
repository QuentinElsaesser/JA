...
class PlotMaxMin():
    ...
    def plot_one(self, i, d=0, t=-1, height=20, width=10, num_met=0, res=[]):
        ...
        for i in range(len(possibv)):
            if i == 0:
                self.index_m = 0
                mini,maxi,items = self.myplot(x=self.percent, y=res[0][i], idv=possibv[i], i=i, j=0, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
                indcol += 1
                self.index_m = 1
                mini,maxi,items = self.myplot(x=self.percent, y=res[1][i], idv=possibv[i], i=i, j=1, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
                indcol += 1
                self.index_m = 3
                mini,maxi,items = self.myplot(x=self.percent, y=res[2][i], idv=possibv[i], i=i, j=3, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
                indcol += 1
