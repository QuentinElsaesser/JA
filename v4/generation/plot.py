...
class Plot():
    ...
    def parameters_plot(self, i, j, ind_opt):
        ...
        """
        if not constants.FORMULA and self.options[ind_opt][0].startswith('c') and j==2:
            if i == 0:
                return "Pl"
            elif i == 2:
                return "Bo"
    ...
    def title(self, i):
        ...
        if self.nvalue == "FCT":
            ...
            if self.distrib != "":
                ...
                tmp = ""
            res += f"{self.infograph[1]} src - {self.infograph[2]}-{self.infograph[3]} fct - {self.infograph[0]} obj - {tmp}"
        return res
        ...
        while line:
            ...
            if line.startswith("\subsection"):
                ...
                if self.nvalue == "PRC" or self.nvalue == "PRP":
                    ...
                    if self.nvalue == "SDR" or self.nvalue == "SPR":
                        tmpprc = f"{tmpprc}"
                    else:
    ...
    def condition(self, ind_method, i):
        ...
        if self.index_m in list(constants.ID_METRICS_SRC.values()):
            if self.nvalue == "PRP" or self.nvalue == "VRN" or self.nvalue == "SPR" or self.nvalue == "SDR":
                if ind_method > 3:
            ...
            if self.nvalue == "PRP" or self.nvalue == "VRN" or self.nvalue == "SPR" or self.nvalue == "SDR":
                ...
                #     return False
        if not constants.FORMULA and self.options[i][0].startswith('c') and (ind_method == constants.ORDER.index(constants.NAMES[1]) \
    ...
    def plot_one(self, i, d=0, t=-1, height=20, width=10):
        ...
        if self.nvalue == "SDR" or self.nvalue == "SPR":
            ...
            ax.legend([handles[idx] for idx in ordermet],[labels[idx] for idx in ordermet], fontsize=fontsize)
            yrange = []
